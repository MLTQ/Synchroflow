...
model.eval()  # Set the model to evaluation mode before conversion

# dummy_input = torch.randn(1, 10, data_normalized.shape[1])  # Dummy input with the correct shape


# device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# dummy_input = torch.randn(1, 10, data_normalized.shape[1]).to(device)
...
# Convert to TFLite
edge_model = ai_edge_torch.convert(model, (dummy_input,))  # Convert model
edge_model.export("transformer_model.tflite")  # Export as TFLite model

print("Model successfully converted to TensorFlow Lite and saved as 'transformer_model.tflite'")
